...
for iO=1:nn
    ...
    [Dirxyz,UnitOffset,Dirxyz.msk] = Locate_Crack(Dirxyz);
    if ~isfield(Maps,"msk")
        Maps.msk = Dirxyz.msk;
    end
    % prepare and run abaqus cae
    [Abaqus,~] = PrintRunCode(Dirxyz, ...
        Dirxyz.msk,Maps.results,ceil(min(size(Dirxyz.X))*0.5-2),UnitOffset);
...
saveas(gcf, [Maps.results '\J_KI_II_III.fig']);
saveas(gcf, [Maps.results '\J_KI_II_III.tif']);    %close all

save([Maps.results '\DIC2CAE.mat'],'Maps','J','KI','KII',...
    'KIII','Direction');
end
